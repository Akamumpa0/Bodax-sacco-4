...
export async function recordSaving(payload, recordedBy) {
  const { rows } = await query(
    `INSERT INTO savings_transactions (member_id, recorded_by, amount, transaction_date, notes)
     VALUES ($1, $2, $3, COALESCE($4, CURRENT_DATE), $5)
     RETURNING *`,
    [payload.member_id, recordedBy, payload.amount, payload.transaction_date || null, payload.notes || null],
  );
  return rows[0];
}
...
export async function statement(memberId, from, to) {
  const { rows } = await query(
    `SELECT 'saving' AS type, amount, transaction_date AS date, notes
     FROM savings_transactions WHERE member_id = $1 AND transaction_date BETWEEN $2 AND $3
     UNION ALL
